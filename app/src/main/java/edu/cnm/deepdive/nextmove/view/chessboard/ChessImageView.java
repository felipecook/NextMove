  ...
  public void onDraw(Canvas canvas) {
    ...
    if (hasFocus()) {
      ...
      if (_colorScheme == 6) { // 6 is color picker
        _paint.setColor(ico._fieldColor == 0 ? pref.getInt("color2", 0xffdddddd)
        ...
        _paint.setColor(ico._fieldColor == 0 ? _arrColorScheme[_colorScheme][0]
            : _arrColorScheme[_colorScheme][1]);
        canvas.drawRect(new Rect(0, 0, getWidth(), getHeight()), _paint);
        if (ico._selected) {
    ...
    if (ico._bPiece) {
      ...
      if (_sActivity.equals(getContext().getString(R.string.start_play))) {
        if (options.is_sbFlipTopPieces()) {
          if ((options.is_sbPlayAsBlack() ? ico._color == 1
              : ico._color == 0)) {   // flips top pieces for human vs human without
